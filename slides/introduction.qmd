...
2. find the slides in `docs/slides/`^[these are generated from the files in slides/, confusingly]
...
```{python pcplots}
#|echo: false
p1 = p9.ggplot(pcdf2, p9.aes(x="PC1", y="PC2", color="source")) + p9.geom_point(size=3, show_legend=False)
p2 = p9.ggplot(pcdf2, p9.aes(x="PC2", y="PC3", color="source")) + p9.geom_point(size=3)
p1 | p2
```
